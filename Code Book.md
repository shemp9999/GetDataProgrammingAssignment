...
The data has been tidied and is in a wide format. There are 68 columns and 180 rows. Each row contains an Activity name, a Subject number and a set of observations for 66 features. Each observation is the average for that column's feature for the given Activity and Subject. Columns are the mean and standard deviation for each of the 17 given signals, including any separate XYZ directions (More data in section 'Feature Selection' below).
